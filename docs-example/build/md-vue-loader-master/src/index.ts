...
// loader query params
type QueryOptions =
  | Partial<loaderUtils.OptionObject>
  | {
...
const convert: loader.Loader = function (source) {
  ...
  if (queryParams.fence) {
    ...
    const index: number =
      typeof queryParams.componentIndex === "string"
        ? isNaN(Number(queryParams.componentIndex))
          ? 0
          : Number(queryParams.componentIndex)
        : 0;
    // match the `vue demo code`
    const demoReg: RegExp = new RegExp(
      `:::${options.containerName || "demo"}[\\s\\S]*?:::`,
      "ig"
    );
    ...
    if (!matches || !matches[index]) {
      console.log(
        `${colors.warn(
          "[markdone-vue-loader]"
        )}: resolve ${index}th ${colors.error(
          ":::demo"
        )} in ${resourcePath} failed`
      );
      return "";
    }
    ...
    const vueBlocks: null | RegExpMatchArray = matches[index].match(
      /```([\s\S]*?)(<[\s\S]*)```/i
    );
    ...
    console.log(
      `${colors.warn(
        ...
        ":::demo"
      )} markded vue code failed in ${resourcePath}.`
    );
    return "";
  }
  ...
  config(
    ...
    function () {
      ...
      // import component
      srciptImport += `import ${componentName} from ${loaderUtils.stringifyRequest(
        loaderContext,
        request
      )};`;
